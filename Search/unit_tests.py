...
from search import dfs, ids, astar, id_astar, tilegame_heuristic
...
class IOTest(unittest.TestCase):
    ...
    def test_id_astar_output(self):
        self._check_algorithm(lambda p: id_astar(p, lambda s: 0))
        self._simple_problem(lambda p: id_astar(p, lambda s: 0), True)
        self._check_dgraph(lambda p: id_astar(p, lambda s: 0))
