...
class SARSA(Tabular_SARSA.Tabular_SARSA):
    ...
    def learn_policy(
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                if done is True:  # if you reach the goal state
                    ...
                    self.etable = self.etable*gamma*lambda_value
                    self.etable[state][action] = 1
                    self.qtable = self.qtable + learning_rate*delta*self.etable
                    self.etable = np.zeros((self.num_states, self.num_actions))
                    break
        ...
        np.save("qvalues_taxi_sarsa_lambda_grading.npy", self.qtable)
        np.save("policy_taxi_sarsa_lambda_grading.npy", self.policy)
