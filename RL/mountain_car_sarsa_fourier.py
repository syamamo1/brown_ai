...
def phi(state):
    ...
    for i in range(num_ind):
        ans.append(np.cos(np.pi*np.dot(multipliers[i], state)))
...
def create_multipliers():
    ...
    multipliers = []
    for i in range(order+1):  # construct multipliers
...
def SARSA_Learning():
    ...
    for ep in range(num_episodes):
        ...
        for t in range(num_timesteps):
            ...
            e *= gamma*Lambda
            e[:, action] = e[:, action] + phi(normalize_state(state))
            if done:  # if at terminal state
                delta = reward - action_value(state, action)
                weights = weights + e*alpha*delta
                e *= 0
                break
            else:  # not at terminal state
                delta = reward - action_value(state, action) + gamma*action_value(next_state, next_action)
                weights = weights + e * alpha * delta
                state = next_state
                action = next_action
