...
some function f: X --> Y. Throughout this stencil, we mean "f"
to be the underlying function to be estimated.
"""


class SupervisedLearner(metaclass=ABCMeta):
	"""
	INSTANCE VARIABLES:
	...
	def compute_features(self, x):
		...
		"""
		featurelist = [self.feature_funcs[i](x) for i in range(len(self.feature_funcs))]
